...
CHARACTER_CARDS: Dict[str, dict] = {
    ...
    "family": {
        "label": "Family",
        "core_style": "a caring family member with gentle, practical support",
    ...
    "pet": {
        "label": "Pet",
        "core_style": "a playful, loyal pet companion tone",
...
PRIMARY_PERSONAS = ["romantic", "friend", "family", "pet"]
...
<body>
  ...
  <div class="grid">
    <div>
      ...
      <select id="persona">__PERSONA_OPTIONS__</select>
    </div>
    <div>
...
async function sendChat() {
  ...
  if (!personaValue) { setStatus('Persona list is empty. Reload page and check backend logs.'); return; }
  if (!message) { setStatus('Please type a message first.'); return; }
  const payload = {
  ...
  try {
    ...
    if (!res.ok) { appendMessage('bot', 'Error: ' + JSON.stringify(data)); setStatus('Request failed. Check Ollama and API logs.'); return; }
    let botText = data.reply + '\n\nFollow-up: ' + data.follow_up_question;
    if (data.safety_note) { botText += '\n\nSafety: ' + data.safety_note; }
...
def render_chat_ui_html() -> str:
    """Render chat UI HTML with persona options from backend source-of-truth."""

    ordered = PRIMARY_PERSONAS + [k for k in CHARACTER_CARDS.keys() if k not in PRIMARY_PERSONAS]
    options = []
    for index, key in enumerate(ordered):
        selected = ' selected' if index == 0 else ''
        label = CHARACTER_CARDS[key].get('label', key)
        options.append(f'<option value="{key}"{selected}>{key} ({label})</option>')
    return CHAT_UI_HTML_TEMPLATE.replace('__PERSONA_OPTIONS__', '\n        '.join(options))
...
async def list_personas() -> dict:
    ...
    return {
        ...
        "all_personas": list(CHARACTER_CARDS.keys()),
...
async def chat(payload: ChatRequest) -> ChatResponse:
    ...
    if payload.persona in ("pet", "pet_companion"):
